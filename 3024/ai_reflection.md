...
| OJ submission ID, if submitted | 560358 |
| OJ status | Pass |
...
```text
i asked ai for a mathematical way to calculate min score 
```
...
```text
its make me realized that its not to find min score its just min score as possible
```
...
```text
i make new math operation to calculate the lowest score as possible not directly find min score
```
...
```text
i learn that math is important for programming and think carefully
```
