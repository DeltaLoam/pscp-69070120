...
```text
its a program to calculate price in school cafeteria to chekc for promotion

input: buy detail and membership details
output: final price
```
...
```text
get input of membership details then use for loop to get what they buy then sum it and calculate the discount by using if else and use math to calculate the price then print it out
```
...
```text
non menbers get discount check
```
