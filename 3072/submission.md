...
```text
check input if there a vowels and then cout and tell how much each one have

input: string
output:  count of howmany each vowels have in input string
```
...
```text
use for loop to check each char if in in aeiou or not then keep it in dict and after that i check each dict if it more than 0 i print it
```
